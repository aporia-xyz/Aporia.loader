...
pub fn maven_to_url(maven: &str, classifier: &str, base_url: &str) -> Option<String> {
    let mut clean_maven = maven.to_string();
    if let Some(last_colon) = maven.rfind(':') {
        if let Some(second_colon) = maven.find(':') {
            if let Some(sc2) = maven.find(':').and_then(|p| maven.find(':').map(|_| maven.find(':').unwrap() + 1).and_then(|p2| maven.find(':').map(|_| p2))) {
                // Нормализация
            }
        }
    }
    ...
    Some(format!("{}{}/{}/{}/{}", base_url, group_path, artifact, version, filename))
}
...
pub fn maven_to_path(maven: &str, classifier: &str, base_path: &str) -> Option<String> {
    ...
    Some(format!("{}/{}/{}/{}/{}", base_path, group_path, artifact, version, filename))
}
